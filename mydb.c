...
extern unsigned char obuf[3][36];
...
int valctl()//控制环境变量数据的插入及定时删除多余的数据
{
	...
	if(NULL != ppTable)
	{
		...
		if(iRow>4)
		{
			char sqldelete[] = "delete * from environment";
			if (DB_ERROR_OK != ExecSQL(sqldelete))
			{
	...
	*/
	...
	printf("数据库要插入时的温度，湿度，光照为%s\t %s\t %s\n",obuf[0],obuf[1],obuf[2]);
	
	int ret=InitBbash(obuf[0],obuf[1],obuf[2]);
	if(ret==DB_ERR)
	{
		printf("插入环境变量失败\n");
		return DB_ERR;
