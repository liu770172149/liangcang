...
int mysocket()
{
			...
			else if (FD_ISSET(i, &readFds))
			{
				...
				if (ret > 0)
				{
					...
					if(ret1 == -3)
					{
						...
						{
							int tmp2=i;
								printf("进入 C 查询,，正在查询。。。。。。id is %d\n",i);
								
							int ret4=pthread_create(&th1,NULL,threadfun1,NULL);
                            if(ret4 !=0)
                            {
                                printf("创建loopctl循环控制变量线程失败 error\n");
                                return SEL_ERR;
                            }
                            if(ret4 == 0)
                            {
                                printf("创建loopctl()//循环控制变量线程成功\n");
								//创建查询线程
								int ret5=pthread_create(&th2,NULL,findall,&tmp2);
								if(ret5 !=0)
								{
									printf("创建查询线程失败 error\n");
									return SEL_ERR;
								}
								if(ret5 == 0)
								{
	...
	int *b;
    pthread_join(th1,(void **)&b);
	pthread_join(th2,(void **)&b);
    printf("查询传输线程结束，状态为%d\n",*b);
	return SEL_OK;
}
